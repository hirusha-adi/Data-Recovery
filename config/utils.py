...
class Utils:
    @staticmethod
    def exec_n_save(command: list[str], output_file: Path, module_name="exec_n_save") -> None:
        logger.info(f"[{module_name}] Running command: {' '.join(command)}")
        try:
            ...
            logger.info(f"[{module_name}] Output saved to {output_file}")
        except subprocess.CalledProcessError as e:
            logger.error(f"[{module_name}] Command failed: {e}")
